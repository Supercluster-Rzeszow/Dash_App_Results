...
def AscentLoop(height):
    ...
    data = pd.DataFrame(np.transpose(np.array([TimeVector, h, v, a, T, p, rho, g, F_G, F_B, F_D, V_b, A_b, r])),
                        columns=['Czas [s]', 'Wysokość [m]', 'Prędkość pionowa [m/s]', 'Przyspieszenie[m/s^2]',
                                 'Temperatura [K]', 'Cisnienie [Pa]', 'Gestosc [kg/m^3]',
                                 'Przyspieszenie grawitacyjne [m/s^2]', 'Siła ciężkości [N]', 'Siła wyporu [N]',
                                 'Siła oporu aerodynamicznego [N]', 'Objetosc balonu [m^3]',
                                 'Powierzchnia czolowa balonu [m^2]', 'Promien balonu [m]'])
